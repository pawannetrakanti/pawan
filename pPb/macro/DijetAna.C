...
const int knj = 2;
const char *calgo[knj] = {"ak3PF"    ,"akPu3PF"};
const double kDelR[knj]  = {0.3,0.3};
...
TStopwatch timer;
int DijetAna(int leadb=2,int sleadb=0,const char *ksp="ppb")
{
  ...
  //! Output file
  //! HIHighPt
  TFile *fout = new TFile(Form("output/promptreco/BiasStudy/BiasStudy_%s_Lead%s_SubLead%s_v77.root",ksp,clead[leadb],cslead[sleadb]),"RECREATE");  
  ...
  TH3F *hypt2pt1   [knj][2][lbins*lbins];
  TH3F *hdedp      [knj][2][lbins*lbins];
  TH3F *hdijept2pt1[knj][2][lbins*lbins];

  for(int nj=0;nj<knj;nj++){
    for(int j=0;j<3;j++){

      //std::cout<<Form("Histograms for algo %s",calgo[nj])<<std::endl;
      hjetpt [nj][j] = new TH3F(Form("hjetpt%d_%d" ,nj,j) ,Form("Jet pt sepctra %s %d",calgo[nj],j),nmult,-0.5,nmult-0.5,lbins*lbins,-0.5,lbins*lbins-0.5,500,0,1000);
      hjeteta[nj][j] = new TH3F(Form("hjeteta%d_%d",nj,j),Form("Jet eta %s %d",calgo[nj],j),nmult,-0.5,nmult-0.5,lbins*lbins,-0.5,lbins*lbins-0.5,12,-1.*ketacut,ketacut);
      hjetphi[nj][j] = new TH3F(Form("hjetphi%d_%d",nj,j),Form("Jet phi %s %d",calgo[nj],j),nmult,-0.5,nmult-0.5,lbins*lbins,-0.5,lbins*lbins-0.5,30,-pi,pi);

      if(j<2){
	for(int il=0;il<lbins;il++){
	  for(int sil=0;sil<lbins;sil++){
	    int ibin  = il*lbins + sil;
	    hypt2pt1   [nj][j][ibin] = new TH3F(Form("hypt2pt1%d_%d_%d",nj,j,ibin),Form("y^{*} pT2/pT1 %s %d %s %s",calgo[nj],j,ctrig[il],cassoc[sil]),nmult,-0.5,nmult-0.5,144,-2.*ketacut,2*ketacut,50,0,1);
	    hdijept2pt1[nj][j][ibin] = new TH3F(Form("hdijept2pt1%d_%d_%d",nj,j,ibin),Form("(eta1+eta2)/2 vs pT2/pT1 %s %d %s %s",calgo[nj],j,ctrig[il],cassoc[sil]),nmult,-0.5,nmult-0.5,72,-1.*ketacut,ketacut,50,0,1);
	    hdedp      [nj][j][ibin] = new TH3F(Form("hdedp%d_%d_%d",nj,j,ibin),Form("deta-dphi %s %d %s %s",calgo[nj],j,ctrig[il],cassoc[sil]),nmult,-0.5,nmult-0.5,144,-2.*ketacut,2*ketacut,144,0,pi);
	  }
	}
	hpt2pt1 [nj][j] = new TH3F(Form("hpt2pt1%d_%d",nj,j),Form("Jet pT2 / pT1 lead pT bins %s %d ",calgo[nj],j),nmult,-0.5,nmult-0.5,lbins*lbins,-0.5,lbins*lbins-0.5,10,0,1);
        hdphi   [nj][j] = new TH3F(Form("hdphi%d_%d"  ,nj,j),Form("Jet dphi lead pT bins %s %d",calgo[nj],j),nmult,-0.5,nmult-0.5,lbins*lbins,-0.5,lbins*lbins-0.5,30,0,pi);
        hdjeta  [nj][j] = new TH3F(Form("hdjeta%d_%d" ,nj,j),Form("di-jet lead pT bins eta %s %d",calgo[nj],j),nmult,-0.5,nmult-0.5,lbins*lbins,-0.5,lbins*lbins-0.5,12,-1.*ketacut,ketacut);

        hdeta   [nj][j] = new TH3F(Form("hdeta%d_%d"  ,nj,j),Form("Jet deta lead pT bins %s %d",calgo[nj],j),nmult,-0.5,nmult-0.5,lbins*lbins,-0.5,lbins*lbins-0.5,144,-2.*ketacut,2*ketacut);
  ...
  for (Long64_t ievt=0; ievt<nentries;ievt++) {//! event loop
    ...
    int msel=-1;
    bin[++msel]  = GetNtracksBin(hiNtracks);
    bin[++msel]  = GetNPixBin(hiNpix);
    bin[++msel]  = GetNPixelTracksBin(hiNpixelTracks);
    bin[++msel]  = GetHFBin(hiHF);
    bin[++msel]  = GetAnaBin(hiHFplusEta4);
    bin[++msel]  = GetNewAnaBin(hiHFSum);
    bin[++msel]  = GetHFPE4Bin(hiHFplusEta4);
    bin[++msel]  = GetHFME4Bin(hiHFminusEta4);
    bin[++msel]  = GetHFPBin(hiHFplus);
    bin[++msel]  = GetHFMBin(hiHFminus);

    //! ZDC bad runs
    bool badRun = RunNo==210658 || RunNo==210676 || RunNo==210635 || RunNo==210638;
    if(!badRun){
      bin[++msel]  = GetZDCBin(hiZDC);
      bin[++msel]  = GetZDCPBin(hiZDCplus);
      bin[++msel]  = GetZDCMBin(hiZDCminus);
    }else{
      bin[++msel]  = -1;
      bin[++msel]  = -1;
      bin[++msel]  = -1;
    }
    bin[++msel]  = 0; //! 13  inclusive (0-100%) 
	...
	if(lead_jet[leadb]){
	  ...
	  for(int ij=0; ij<mJets->nref; ij++){
	    ...
	    bool seljet = false;
	    if(strcmp(ksp,"ppb")==0){
	      seljet = mJets->jtpt[ij]>kptrecocut && mJets->rawpt[ij]>15. && fabs(mJets->jteta[ij])<ketacut && mJets->trackMax[ij] > 4.;
	    }else seljet = mJets->jtpt[ij]>kptrecocut && mJets->rawpt[ij]>15. && fabs(mJets->jteta[ij])<ketacut && (mJets->trackMax[ij]/mJets->jtpt[ij]) > 0.01;
	    if(!seljet)continue;
