...
TStopwatch timer;
void ForRaa_Fit(const char *ksp="pp")
{
  ...
  TFile *fout = 0;
  if(strcmp(ksp,"pp")==0){
    fout = new TFile(Form("Output/HLT_Jet60_v1_ppSmearing_Fit_HiForest2_%s_pt%0.0f_2012.root",ksp,kptrecocut),"RECREATE");  
  ...
  */
  ...
  TH1F *hjetrpt  [knj][ncen], *hjetjpt[knj][ncen], *hjetjpu[knj][ncen];
  ...
  TH2F *hjetptbkgd_pftower[knj][ncen];
  ...
  for(int nj=0;nj<knj;nj++){
    ...
    for(int icen=0;icen<ncen;icen++){
      hJetEnergyScale[nj][icen] = new TH2F(Form("hJetEnergyScale%d_%d",nj,icen),Form("hJetEnergyScale%d_%d",nj,icen),bins,ptbins,50,0,5);
      ...
      hjetspt_Syslo[nj][icen] = new TH1F(Form("hjetspt_Syslo%d_%d",nj,icen),Form("Systematic low 5 percent jet(smeared pt) p_{T} distribution jet centb %d %s",icen,calgo[nj]),bins,ptbins);
      hjetspt_Syshi[nj][icen] = new TH1F(Form("hjetspt_Syshi%d_%d",nj,icen),Form("Systematic hi 5 percent  jet(smeared pt) p_{T} distribution jet centb %d %s",icen,calgo[nj]),bins,ptbins);
      ...
      hjetptbkgd[nj][icen] = new TH2F(Form("hjetptbkgd%d_%d",nj,icen),Form("jet(pt:bkgd) distribution jet centb %d %s",icen,calgo[nj]),bins,ptbins,100,0,300);
  ...
  for (Long64_t ievt=0; ievt<nentries;ievt++) {//! event loop
	...
	if(nj==0)mJets = &(c->akPu2PF);
      ...
      */
      ...
      if(ljet[0]>=0 && ljet[1]>=0){//! atleas a dijet
	int mstat=1;
	double ptdij = (mJets->jtpt[ljet[0]] + mJets->jtpt[ljet[1]])/2.;
	if(ljet[2]>=0){
	  if(mJets->jtpt[ljet[2]]/ptdij > 0.1)mstat=0;
	}
	if(mstat){
	  double B = (mJets->jtpt[ljet[0]] - mJets->jtpt[ljet[1]])/ptdij;
	  hJetEnergyScale[nj][centb]->Fill(B,mJets->jtpt[ljet[0]]/mJets->jtpt[ljet[1]]);
	}
      }
      delete [] ljet;
	...
	if(fabs(mJets->jteta[ij]) < ketacut){
	  ...
	  if(strcmp(ksp,"pp")==0){
	    for(int ic=0;ic<ncen-1;ic++){
	      float smpt    = GetSmearedPtData(nj+1,ic,mJets->jtpt[ij],0,"no");
	      if(smpt>=kptrecocut)hjetspt [nj][ic]->Fill(smpt);

	      float losmpt  = GetSmearedPtData(nj+1,ic,mJets->jtpt[ij],5,"low");
	      if(losmpt>=kptrecocut)hjetspt_Syslo [nj][ic]->Fill(losmpt);

	      float upsmpt  = GetSmearedPtData(nj+1,ic,mJets->jtpt[ij],5,"up");
	      if(upsmpt>=kptrecocut)hjetspt_Syshi [nj][ic]->Fill(upsmpt);
	    }
	  }
