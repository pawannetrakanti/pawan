...
public :
   ...
   Int_t           hiNevtPlane;
   Float_t         hiEvtPlanes[MaxNumber];   //[hiNevtPlane]
...
void setupEvtTree(TTree *t,Evts &tEvts,bool doCheck = 0)
{
  ...
  t->SetBranchAddress("hiEvtPlanes", tEvts.hiEvtPlanes, &tEvts.b_hiEvtPlanes);
   ...
   if (doCheck) {
     if (t->GetMaximum("hiNevtPlane")>38) cout <<"FATAL ERROR: Arrary size of hiNevtPlane too small!!!  "<<t->GetMaximum("hiNevtPlane")<<endl;
   }
}
